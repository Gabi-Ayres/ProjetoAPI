...
export const getTasksByTagId = (tagId)=> {
    const filteredTags = taskService.getTaskTags().filter((tt) => tt.tagId === parseInt(tagId));
    const filteredTasks = filteredTags.map(t => t.taskId);
    const filteredTasksTags = taskService.getAllTasks().filter((tt) => filteredTasks.includes(tt.id));
...
export const deleteTag = (tagsId) => {
  taskService.taskTags = taskService.taskTags.filter((tt) => tt.tagId !== parseInt(tagsId));
